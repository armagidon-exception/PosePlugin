...
public class LayPose extends PluginPose {
    ...
    private EntityPlayer vanilla = FakePlayer.asNMSCopy(getPlayer());
    private PacketPlayOutEntityMetadata metadata = new PacketPlayOutEntityMetadata(vanilla.getId(), vanilla.getDataWatcher(), false);
    ...
    private void disappear(){
        ...
        vanilla.playerConnection.sendPacket(metadata);
        getPlayer().hidePlayer(PosePlugin.getInstance(), getPlayer());
    }

    private void appear(){
        vanilla.setInvisible(false);
        vanilla.playerConnection.sendPacket(metadata);
